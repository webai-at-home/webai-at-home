...
Test('validates every inbound client message shape', () => {
	...
	// nothing unusual, and a consumer drops it rather than carrying it. Protocol version 11 made it
	// carry the schema itself, so that a worker has the thing it is meant to enforce. See issue #219.
	...
	for (const responseFormat of [
		...
		{ type: 'text' },
		{ type: 'yaml' },
		// A schema request without a schema, which is a request to follow nothing.
		{ type: 'json_schema', name: 'greeting' },
		// A name is what the OpenAI Chat Completions interface requires beside a schema, so an empty
		// one is refused rather than passed on to a local server that would refuse it.
		{ type: 'json_schema', name: '', schema: { type: 'object' } },
		// A `json_object` request carries nothing else, so a schema beside it is a request nobody read.
		{ type: 'json_object', schema: { type: 'object' } },
	]) {
		Assert.equal(ClientMessageSchema.safeParse({ type: 'task.submit', taskRequestId: 'request-1', input: { taskType: 'task_type_llm_gemma_4_e2b_full', input: 'hello', generationSettings: { responseFormat } } }).success, false, JSON.stringify(responseFormat));
	}
...
Test('names both shapes as honoured only by the task type both of whose workers were measured producing them', () => {
	...
	// ask its local server for the shape and refuse an answer that came back as anything else, and
	// widened again by milestone 6, which made the schema itself travel and gave both workers the same
	// reader to enforce it with.
	...
	Assert.deepEqual(StructuredOutputSupport.honouredFormats('task_type_llm_gemma_4_e2b_full'), ['json_object', 'json_schema']);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//	JsonSchemaCompiler and JsonSchemaGrammar
//
//	The schema a `json_schema` request carries, turned into something a worker can enforce one
//	character at a time, and the reader that enforces it. Both come from milestone 6 of
//	[issue #219](https://github.com/webai-at-home/webai-at-home/issues/219), and both live here
//	rather than in a worker because three places have to agree about what a schema means: the
//	consumer refusing a schema no worker could keep, the worker browser tab masking every token that
//	would break it, and the native worker checking what its local server sent back. Two copies of
//	this would drift, and the drift would be answered with an object reported as matching a schema
//	it does not match.
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

/**
 * Reads a whole text against one schema, and reports whether it was accepted and finished.
 *
 * @param schema The schema the text has to satisfy.
 * @param text The text to read.
 * @returns Whether every character was accepted, and whether a complete value was written.
 */
function readAgainstSchema(schema: Record<string, unknown>, text: string): { isAccepted: boolean; isComplete: boolean } {
	const nodes = JsonSchemaCompiler.compile(schema);
	const state = JsonSchemaGrammar.initialState(0);
	const isAccepted = JsonSchemaGrammar.acceptText(nodes, state, text);
	return {
		isAccepted: isAccepted,
		isComplete: isAccepted === true && JsonSchemaGrammar.isComplete(nodes, state),
	};
}

/** The schema every reading check below is made against, holding each kind the compiler enforces. */
...
Test('compiles a schema into one node per value the answer may hold', () => {
	const nodes = JsonSchemaCompiler.compile(greetingSchema);

	Assert.equal(nodes[0].kind, 'object');
	Assert.deepEqual(nodes[0].propertyNames, ['greeting', 'count', 'mood', 'tags']);
	Assert.equal(nodes[0].allowsOtherProperties, false);
	// One bit per property, set for `greeting` at index 0 and `mood` at index 2.
	Assert.equal(nodes[0].requiredMask, 0b0101);
	Assert.deepEqual(nodes[nodes[0].propertyNodeIndexes[2]].enumTexts, ['"happy"', '"sad"']);
	Assert.equal(nodes[nodes[0].propertyNodeIndexes[3]].kind, 'array');
});

Test('refuses a schema it could not hold an answer to, rather than enforcing the part it understands', () => {
	// A keyword left unenforced is the failure this whole table exists to prevent: an answer that
	// broke `minLength` would come back reported as matching the schema. So a schema carrying one is
	// refused where it is read, and the consumer answers the request with an error.
	for (const [schema, expected] of [
		[{ type: 'string', minLength: 3 }, /uses "minLength"/],
		[{ type: ['string', 'null'] }, /a list of types/],
		[{ type: 'lemon' }, /declares the type "lemon"/],
		[{ type: 'object', required: ['a'] }, /requires properties without declaring any/],
		[{ type: 'object', properties: { a: { type: 'string' } }, required: ['b'] }, /requires the property "b"/],
		[{ type: 'object', anyOf: [] }, /uses "anyOf"/],
	] as [Record<string, unknown>, RegExp][]) {
		Assert.throws(() => JsonSchemaCompiler.compile(schema), expected, JSON.stringify(schema));
	}
});

Test('reads an answer that satisfies the schema, and refuses one that breaks any part of it', () => {
	for (const text of [
		'{"greeting":"hello","mood":"happy"}',
		'{"greeting": "hi", "count": 3, "mood": "sad", "tags": ["a", "b"]}',
		'{"greeting":"hello","mood":"happy","tags":[]}',
		'{"greeting":"hello","count":-12,"mood":"sad"}',
	]) {
		Assert.deepEqual(readAgainstSchema(greetingSchema, text), { isAccepted: true, isComplete: true }, text);
	}

	for (const text of [
		// A required property left out. The value reads as legal JSON and is refused as incomplete,
		// because nothing is wrong with it until the closing brace says no more is coming.
		'{"greeting":"hello"}',
		// A property the schema never declared, where `additionalProperties` is false.
		'{"greeting":"hello","mood":"happy","extra":1}',
		// A value of the wrong type: a string where an integer was declared, and the other way round.
		'{"greeting":"hello","mood":"happy","count":"3"}',
		'{"greeting":3,"mood":"happy"}',
		// An integer written with a fraction, which JSON accepts and `"type": "integer"` does not.
		'{"greeting":"hello","mood":"happy","count":1.5}',
		// A value outside the enumeration, and one that only starts like a member of it.
		'{"greeting":"hello","mood":"angry"}',
		'{"greeting":"hello","mood":"happyish"}',
		// An array item of the wrong type.
		'{"greeting":"hello","mood":"happy","tags":[1]}',
		// The same property written twice.
		'{"greeting":"hello","greeting":"hi","mood":"happy"}',
		// The whole answer of the wrong kind, where the schema declared an object.
		'["hello"]',
	]) {
		const reading = readAgainstSchema(greetingSchema, text);
		Assert.equal(reading.isAccepted === true && reading.isComplete === true, false, text);
	}
});

Test('leaves legal, at each step, only the characters the schema still allows there', () => {
	// This is what a mask is built from, so a reader that allows one character too many makes the
	// mask a decoration. What it allows is asked here directly, one character at a time.
	const legalCharactersAfter = (schema: Record<string, unknown>, written: string): string => {
		const nodes = JsonSchemaCompiler.compile(schema);
		const state = JsonSchemaGrammar.initialState(0);
		Assert.equal(JsonSchemaGrammar.acceptText(nodes, state, written), true, written);
		const legal: string[] = [];
		for (let code = 32; code < 127; code = code + 1) {
			if (JsonSchemaGrammar.acceptsText(nodes, state, String.fromCharCode(code)) === true) {
				legal.push(String.fromCharCode(code));
			}
		}
		return legal.join('');
	};

	// An object is the only thing the answer may open with, and a required property is the only key
	// it may open, so the model is handed the first letter of that key and nothing else.
	Assert.equal(legalCharactersAfter(greetingSchema, ''), ' {');
	Assert.equal(legalCharactersAfter(greetingSchema, '{"'), 'cgmt');
	// Once every required property is written, the object may close; before that it may not.
	Assert.equal(legalCharactersAfter(greetingSchema, '{"greeting":"hi"'), ' ,');
	Assert.equal(legalCharactersAfter(greetingSchema, '{"greeting":"hi","mood":"sad"'), ' ,}');
	// A key already written is not offered again, so the same property cannot be written twice.
	Assert.equal(legalCharactersAfter(greetingSchema, '{"greeting":"hi","mood":"sad","'), 'ct');
	// Inside an enumeration only the members still possible may continue.
	Assert.equal(legalCharactersAfter(greetingSchema, '{"greeting":"hi","mood":"h'), 'a');
	// A number is where a type is enforced most tightly: an integer may open neither a fraction nor
	// an exponent. `1e2` is the integer 100 and is refused all the same, because a reader that let the
	// exponent open would have to read the whole of it before it could tell 1e2 from 1e-2.
	Assert.equal(legalCharactersAfter(greetingSchema, '{"greeting":"hi","mood":"sad","count":1'), ' ,0123456789}');
});

Test('says nothing about a value the schema says nothing about, and everything about one it does', () => {
	// `json_object` is a request for any object at all, and `{ "type": "object" }` is the schema that
	// says so. Both workers compile it, so there is one grammar rather than two.
	const anyObject = {
		type: 'object',
	};

	Assert.deepEqual(readAgainstSchema(anyObject, '{"whatever":[1,{"deep":null}]}'), { isAccepted: true, isComplete: true });
	Assert.deepEqual(readAgainstSchema(anyObject, '{}'), { isAccepted: true, isComplete: true });
	Assert.equal(readAgainstSchema(anyObject, '[1]').isAccepted, false);
	Assert.equal(readAgainstSchema(anyObject, '{"a":1,}').isAccepted, false);
});
...
Test('judges a token that closes two containers at once against the whole stack, not only its top', () => {
	...
	const nodes = JsonSchemaCompiler.compile({
		type: 'object',
	});
	const state = JsonSchemaGrammar.initialState(0);
	JsonSchemaGrammar.acceptText(nodes, state, '{"a":{"b":1');

	Assert.equal(JsonSchemaGrammar.acceptsText(nodes, state, '}}'), true);
	Assert.equal(JsonSchemaGrammar.acceptsText(nodes, state, '}}}'), false);
	Assert.equal(JsonSchemaGrammar.isComplete(nodes, state), false);
	Assert.equal(JsonSchemaGrammar.acceptText(nodes, state, '}}'), true);
	Assert.equal(JsonSchemaGrammar.isComplete(nodes, state), true);
});

Test('gives one signature to one state, so a mask worked out for it can be reused', () => {
	const nodes = JsonSchemaCompiler.compile(greetingSchema);
	const oneState = JsonSchemaGrammar.initialState(0);
	const anotherState = JsonSchemaGrammar.initialState(0);
	JsonSchemaGrammar.acceptText(nodes, oneState, '{"greeting":"one');
	JsonSchemaGrammar.acceptText(nodes, anotherState, '{"greeting":"another');

	// Two answers, different text, the same place in the schema: one mask serves both.
	Assert.equal(JsonSchemaGrammar.signatureOf(oneState), JsonSchemaGrammar.signatureOf(anotherState));

	const afterTheString = JsonSchemaGrammar.copy(oneState);
	JsonSchemaGrammar.acceptText(nodes, afterTheString, '"');
	Assert.notEqual(JsonSchemaGrammar.signatureOf(oneState), JsonSchemaGrammar.signatureOf(afterTheString));
});
