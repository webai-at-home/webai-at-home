 ...
 *
 * Version 10 let a consumer say what shape it wants its answer in, and carry the schema that shape
 * has to match. `GenerationSettingsSchema` now carries `responseFormat`, holding `json_object` or
 * `json_schema` exactly as the OpenAI Chat Completions interface names them, and a `json_schema`
 * carries the JSON Schema itself rather than the bare name of a shape a worker would then have to
 * invent. This is the first version in which such a request travels further than the consumer that
 * read it: before it, `@webai/consumer-openai` read the field only to refuse a shape no task type
 * could produce, and dropped what it read. Which task type honours which shape stays in
 * `structured_output_support.ts`, so `responseFormat` is not a `GenerationControlName` and is not
 * refused against the generation control table. See milestone 2 of
 * [issue #221](https://github.com/webai-at-home/webai-at-home/issues/221). Every earlier version is
 * refused for the same reason version 9 refuses version 8, and over the same schema:
