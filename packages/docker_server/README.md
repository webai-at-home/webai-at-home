...
| `GATEWAY_TRUST_REVERSE_PROXY` | `false` | the gateway's `--trust-reverse-proxy`, when it is set to `true` — needed in any deployment reached over TLS on a domain name, so each worker is recorded at the address it connected from rather than at the address of the reverse proxy |
...
Deployment is a deliberate step, and not something every push does, because restarting the container disconnects every worker browser tab and every worker process connected to the gateway. What that costs is now small: each of them connects again on its own, waiting one second before the first attempt and longer after each attempt that finds no gateway, up to one minute, so a container that is back within a minute or two collects its volunteers again with nobody touching a device. The work in flight at the moment of the restart is still lost, and a task whose stage was being run is placed again by the gateway. See [issue #158](https://github.com/webai-at-home/webai-at-home/issues/158) for the automatic reconnection, and [issue #149](https://github.com/webai-at-home/webai-at-home/issues/149) for the question of deploying less often that it answers. The gateway's own durable state survives the restart, because the task state file, the account profile file, and the accounting ledger file all live on the `/data` volume described in [Start](#start).
