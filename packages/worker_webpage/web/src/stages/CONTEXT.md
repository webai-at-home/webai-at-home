...
- `chat_template_tools.ts`, `gemma_4_e2b_history_messages.ts`: a history's tool declarations and its messages, in the shape a chat template reads.
- `stop_sequence_watcher.ts`: stopping an answer at a stop sequence without forwarding it.
- `structured_output/`: enforcing a response format's shape — see its own CONTEXT.md.
...
- The Transformers.js stage helpers report an exact `promptTokenCount`, `completionTokenCount`, and `stopReason`; `stage_helper_llm_gemma_nano_chrome_full.ts` reports only `stopReason`, having no token count ([#150](https://github.com/webai-at-home/webai-at-home/issues/150), [#154](https://github.com/webai-at-home/webai-at-home/issues/154)).
...
- Each tool call reader reads one format; Gemma 4 E2B's markers are special tokens, so its stage decodes with `skip_special_tokens: false` ([#115](https://github.com/webai-at-home/webai-at-home/issues/115), [#216](https://github.com/webai-at-home/webai-at-home/issues/216)).
- This folder imports from no folder of this package outside itself.
