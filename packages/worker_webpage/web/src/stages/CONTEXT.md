...
- `stage_helper_llm_qwen3_0_6b_sharded.ts`: runs one Qwen3-0.6B shard stage, whose sampler is written by hand over the logits.
...
- A stage helper samples only for an answer that asked for a temperature above zero or a `topP`, so greedy decoding stays what a consumer asking for nothing receives.
