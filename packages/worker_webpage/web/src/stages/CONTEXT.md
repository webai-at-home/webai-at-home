...
- `stop_sequence_watcher.ts`, `thought_channel_cut.ts`, `empty_answer_refusal.ts`: a stop sequence held back, thinking cut out, an answer that never began refused.
...
- The Transformers.js stage helpers report an exact `promptTokenCount`, `completionTokenCount`, and `stopReason`; `stage_helper_llm_gemma_nano_chrome_full.ts` reports only `stopReason`, always `end_of_sequence`.
...
- A stage helper samples only for an answer asking for a temperature or a `topP`; a consumer asking for nothing gets greedy decoding.
- `stage_helper_llm_qwen3_5_0_8b_full.ts` and `stage_helper_llm_gemma_4_e2b_full.ts` read `reasoningEffort` as `enable_thinking`: `none` off, anything above it on, nothing asked for off. Gemma 4 E2B's thinking is cut out of its answer; Qwen3.5-0.8B's is not ([#192](https://github.com/webai-at-home/webai-at-home/issues/192), [#223](https://github.com/webai-at-home/webai-at-home/issues/223)).
