...
Test('asks for a json_object answer as the schema that means the same thing, which both local servers read', async () => {
	// `{ "type": "json_object" }` is what OpenAI names this shape, and LM Studio 0.4.20 answers it
	// with HTTP 400 and `'response_format.type' must be 'json_schema' or 'text'`. Both it and Ollama
...
Test('sends no response_format field at all when the consumer asked for no shape, so the request is the one it always was', async () => {
	const askedForNothing = await requestBodyOf(undefined);
	Assert.equal('response_format' in askedForNothing, false);
	// Asking for a control and no shape must not carry a shape in either, since a server reading
	// `response_format` would otherwise be told something the consumer never said.
	const askedForOneControl = await requestBodyOf({ temperature: 0 });
...
Test('fails the stage when the local server ignored the shape and answered in prose', async () => {
	// The worker browser tab running this same task type masks every token that would break the
	// shape, so it cannot produce prose here. This worker only asks, and a server that reads
	// `response_format` and ignores it would hand a consumer prose to call `JSON.parse` on.
...
Test('fails the stage when the local server answered with JSON that is not an object', async () => {
	// A bare number and a list are both valid JSON and neither is a JSON object. The worker browser
	// tab's grammar allows only a top-level object, and the two workers of one task type keep one
	// promise between them.
	for (const answer of ['3', '"Paris"', '[1, 2]']) {
...
Test('leaves alone a shaped answer the local server cut short at its output limit, however unfinished its JSON is', async () => {
	// `stopReason: max_new_tokens` already says the answer was cut short, so a consumer receiving
