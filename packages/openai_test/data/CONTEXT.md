...
- A `report:*` script names a real endpoint this project measures — LM Studio, this cluster's own `consumer_openai`, or `api.openai.com`. There is no Ollama script: this project stopped using Ollama, and `packages/openai_conformance_test/data/reports/` keeps the Ollama reports already written.

## Background

- The two folders come from the source tree of [issue #208](https://github.com/webai-at-home/webai-at-home/issues/208), which splits by subcommand what `packages/openai_conformance_test/data/reports/` held for one subcommand alone.
