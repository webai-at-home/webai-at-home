# Directory Context: `/packages/openai_test/examples/typescript`

## Purpose

One short runnable TypeScript program per task type and per calling style, each one showing how a client sends a chat completion request to this project's `consumer_openai` server with the official `openai` package on npm.
