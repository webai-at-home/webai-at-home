...
- A cache is built with the request mode its prober sends every request in, so one run can measure `nostream` and `streamed` separately rather than assuming the two behave alike.
