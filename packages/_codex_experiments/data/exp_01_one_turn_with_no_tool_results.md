...
| WebAI@Home | `http://localhost:8788/v1` | `llm_gemma_4_e2b_full` | 0 | 16 | 8157 | `ready` |

All three target models pass the gate. The plan continues.
...
## WebAI@Home Failed Twice Before It Passed, For Two Different Reasons

**The first reason was a missing endpoint.** The OpenAI-compatible server of `packages/consumer_openai` served `POST /v1/chat/completions` and `GET /v1/models`, and answered `404 Not Found` to `POST /v1/responses`:
...
The Codex command-line program retried five times before giving up. That was a missing endpoint, not a missing worker, and it was reached before any task ever got to a worker. [Issue #214](https://github.com/webai-at-home/webai-at-home/issues/214) added the endpoint.

**The second reason was that every request declares tools, and this model accepted none.** The rerun failed in zero seconds:
...
The name `exp_01_one_turn_with_no_tool` describes the question, not the request. The question needs no tool, and the Codex command-line program declares its ten tools all the same, on this request as on every other, which `exp_03_prompt_size_measure` measured at 18587 characters of the body. `llm_gemma_4_e2b_full` was not one of the task types that accept tool declarations, so the request was refused rather than answered as though no tool had been declared.

**Both are fixed, and the run recorded above is the one that passes.** [Issue #216](https://github.com/webai-at-home/webai-at-home/issues/216) made `task_type_llm_gemma_4_e2b_full` read tool calls, one milestone at a time, and the refusal is gone because the ability it was protecting against now exists rather than because the refusal was weakened. A request declaring tools to a task type that still cannot read them is refused exactly as before.

The 8157 input tokens are the same number LM Studio reported for the same question when it was asked directly, which is what should happen: this run was served by a native `worker_openai` in front of LM Studio's `google/gemma-4-e2b`, so the count is LM Studio's own, carried back through the cluster.
