 ...
 * next.
 *
 * This is `json_grammar.ts` of milestone 1 with the schema added: the same pushdown reader over the
 * same JSON, narrowed at every point by what the schema allows there. Where that reader asked only
 * whether a character is legal JSON, this one asks whether it is legal JSON **and** allowed by the
 * node the reader is inside.
 *
