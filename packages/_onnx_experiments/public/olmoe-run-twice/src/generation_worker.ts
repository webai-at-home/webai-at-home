...
import type { ExpertResidency, ResidentExpert } from './olmoe_generator.js';
import type { ExpertSlot } from './expert_slot_cache.js';
import { ExpertSlotCache } from './expert_slot_cache.js';
import { OlmoeGenerator } from './olmoe_generator.js';
import { StagingBufferRing } from './staging_buffer_ring.js';

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//	GenerationWorker — the whole of milestone 5 of issue #169, in one dedicated worker
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
...
/** The name of the block file in the Origin Private File System. */
const BLOCK_FILE_NAME = 'olmoe_expert_blocks.bin';
...
class GenerationWorker {
	...
	private static _generator: OlmoeGenerator | undefined;
	...
	private static async _handleRequest(scope: DedicatedWorkerScope, request: GenerationRequest): Promise<void> {
		try {
			if (request.kind === 'open-store') {
				scope.postMessage(await GenerationWorker._openStore(request.graphsUrl));
	 ...
	 */
	private static async _openStore(graphsUrl: string): Promise<GenerationResponse> {
	...
	private static async _loadGraphs(): Promise<GenerationResponse> {
		...
		const embedding = new Float32Array(await embeddingResponse.arrayBuffer());
		...
		GenerationWorker._generator = new OlmoeGenerator(index, layerSessions, headSession, expertSession, embedding);
	...
	/**
	 * Runs the model once, with a cache of the size the request asked for.
	 *
	...
	private static async _run(
		...
		// A fresh cache for each run, so neither run starts with anything the other left behind.
		GenerationWorker._cache?.destroy();
		GenerationWorker._cache = new ExpertSlotCache(device, index.expertBlocks.parts, request.slotCount);
		GenerationWorker._readByteLength = 0;

		let preloadMilliseconds = 0;
		if (request.preloadEveryExpert === true) {
		...
		scope.postMessage({
			...
			outcome: {
				...
				everyExpertPreloaded: request.preloadEveryExpert,
	...
	private static async _loadExpert(expertIndex: number, slot: ExpertSlot): Promise<void> {
		...
		const readByteLength = handle.read(new Uint8Array(staging.mapped), {
	 ...
	 */
	private static _requireGenerator(): OlmoeGenerator {
