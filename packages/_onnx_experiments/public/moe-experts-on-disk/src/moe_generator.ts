...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//	MoeGenerator — a whole mixture-of-experts model, assembled from graphs and streamed experts
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////

/**
 * The model loop of milestones 5 and 6 of https://github.com/webai-at-home/webai-at-home/issues/169, copied
 * from the milestone 5 page and widened to cover Qwen3-30B-A3B as well as OLMoE-1B-7B-0924. Copying rather than
 * sharing is this package's rule, and it is what keeps the milestone 5 result reproducible while this one moves.
 *
 * Three things differ between the two models and every one of them is read out of `graphs.json` rather than
 * assumed: how many key and value heads the cache holds, whether the routing weights are divided by their own
 * sum, and whether the embedding table is stored at single precision or at BF16.
 *
...
export class MoeGenerator {
	 ...
	 *
	 * Held in whatever form it was written. Qwen3-30B-A3B's is 151936 rows of 2048, which is 1.16 gigabytes at single
	 * precision and 594 megabytes at BF16, and widening BF16 is an exact shift of sixteen bits, so the narrow form is
	 * kept and one row is widened for each token.
	 */
	private readonly _embedding: Float32Array | Uint16Array;
	/** Whether a row has to be widened from BF16 before it is used. */
	private readonly _embeddingIsBrainFloat: boolean;
	/** Reused for widening one row, so that a generation of many tokens does not allocate one array for each. */
	private readonly _embeddingRow: Float32Array;
	/** The same row seen as raw bits, which is how the sixteen-bit shift is written. */
	private readonly _embeddingRowBits: Uint32Array;
	 ...
	 * @param embedding The token embedding table, of `vocabularySize * hiddenSize` values, in whatever form
	 *   `graphs.json` says it was written.
	 */
	constructor(
		index: ModelIndex,
		...
		embedding: Float32Array | Uint16Array,
	) {
		this._index = index;
		...
		this._embeddingIsBrainFloat = index.tokenEmbedding.elementType === 'bfloat16';
		this._embeddingRow = new Float32Array(index.hiddenSize);
		this._embeddingRowBits = new Uint32Array(this._embeddingRow.buffer);
	...
	async generate(
		...
		// The cache holds the key and value heads. Qwen3-30B-A3B has 32 query heads and only 4 of these, so sizing
		// this from the query head count gives a cache eight times too large that the graph then refuses.
	 ...
	 * @returns A copy of its row at single precision, which the loop then owns and overwrites.
	 */
	private _embed(tokenId: number): Float32Array {
		const width = this._index.hiddenSize;
		if (this._embeddingIsBrainFloat === false) {
			return (this._embedding as Float32Array).slice(tokenId * width, (tokenId + 1) * width);
		}
		// BF16 is the top sixteen bits of a single precision value, so this is a shift rather than a conversion and
		// nothing is lost or invented.
		const stored = this._embedding as Uint16Array;
		for (let channel = 0; channel < width; channel++) {
			this._embeddingRowBits[channel] = stored[tokenId * width + channel] << 16;
		}
		return this._embeddingRow.slice();
	}
	 ...
	 *
	 * The softmax is over every expert of the layer and is taken before the top-k, not after.
	 *
	 * Whether the chosen weights are then divided by their own sum is read out of the model. OLMoE-1B-7B-0924 sets
	 * `norm_topk_prob` false and Qwen3-30B-A3B sets it true, so there is no default that is right for both, and
	 * getting it wrong scales every expert's contribution — by about 2.64 in one direction for OLMoE — while leaving
	...
	private _route(logits: Float32Array): { expertNumber: number; weight: number }[] {
		...
		const chosen = ordered.slice(0, this._index.expertsForEachToken);
		if (this._index.normalizeTopExpertWeights === true) {
			let chosenTotal = 0;
			for (const choice of chosen) {
				chosenTotal += choice.weight;
			}
			for (const choice of chosen) {
				choice.weight /= chosenTotal;
			}
		}
		return chosen;
	}
