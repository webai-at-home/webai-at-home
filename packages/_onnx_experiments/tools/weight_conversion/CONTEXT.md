...
- `gate_quantize_real_expert.ts`: the milestone 3 gate that chose the quantization scheme, by comparing against a published 4-bit conversion of the same model.
- `measure_qwen3_moe_residency.ts`: the milestone 1 arithmetic, read from the real published tensor shapes rather than from `config.json`.
