...
- `src/cli.ts`: the `openai_api_tool` command line program, with five subcommands, implemented in `src/commands/`: `completion`, `history`, `benchmark`, `usage`, and `generation_controls`.
...
- `src/shared_options.ts`: every command line option all five subcommands accept.
