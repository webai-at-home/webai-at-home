...
| `-u, --gateway-url <url>` | `GATEWAY_WS_URL` environment variable, then `wss://webai-gateway.dash-menu.com/` | The WebSocket address of the central gateway. That default is the hosted gateway, so this server joins it without being told to; pass a gateway running on this machine, such as `ws://localhost:8787`, to work against that instead. |
| `-a, --auth-token <token>` | `GATEWAY_AUTH_TOKEN` environment variable, then `development-token` | The bearer token the central gateway requires. The short letter is `-a`, matching `consumer_cli` and `worker_openai`; it was `-t` until [issue #171](https://github.com/webai-at-home/webai-at-home/issues/171), where `-t` also meant `--task_type` in `consumer_cli submit`. |
| `--api-key <key>` | none | The key a request must present to this server, sent in an `Authorization` header as `Bearer` followed by the key. Omitted means no key is required. It has no short letter on purpose: this is a key a caller presents *to this server*, while `worker_openai`'s `-k, --openai-api-key` is a key that program presents *to the local model server*, and one letter cannot mean both directions. See [issue #171](https://github.com/webai-at-home/webai-at-home/issues/171). |
| `--log-dir <path>` | `~/.webai-at-home/consumer_openai/logs` | Where this server's message log and its transaction log are written. Never the directory the server was started from. |
| `-n, --consumer_name <name>` | `consumer_openai server` | The consumer name this server registers under with the central gateway. |
| `-c, --config_dir <path>` | `~/.webai-at-home/consumer_openai_config` | The directory holding this server's own account key pair, as `default.account_key.json`, so the stages its tasks run are recorded against that account. One deployment of this server is one account, and it is this server's account rather than the account of whichever program called its OpenAI-compatible endpoint: this server is what the gateway sees. A directory with no key pair in it means no account, and the stages are recorded against the shared development account instead. See [`docs/accounting_system.md`](../../docs/accounting_system.md). |
...
| `llm_gemma_4_e2b_full` | The complete Gemma 4 E2B instruction-tuned model, downloaded from Hugging Face and held by one worker browser tab. | One worker browser tab with a WebGPU adapter carrying `shader-f16` and enough free storage for the roughly 3111 MB download. This stage has no WebAssembly fallback, so a tab without such an adapter does not offer it at all rather than running it slowly. |
...
The examples that use the official `openai` package on npm against this server live in [`@webai/openai-test`](../openai_test/), in [`packages/openai_test/examples/`](../openai_test/examples), each one runnable on its own. Start with the development formula example, which needs no model download:
...
| `llm_gemma_4_e2b_full` | Always — an exact prompt count from the model's own chat template, and an exact completion count counted as it generates, the same as the two above. |
...
| `llm_gemma_4_e2b_full` | `temperature`, `max_completion_tokens`, `stop`, and `reasoning_effort`. The first three were measured on both of its workers by [issue #222](https://github.com/webai-at-home/webai-at-home/issues/222), and not `top_p` or `seed`, its `@huggingface/transformers` engine acting on neither. `reasoning_effort` was measured by [issue #223](https://github.com/webai-at-home/webai-at-home/issues/223): this is the second model here that thinks before it answers on both of its workers, and both are as coarse as each other, `none` costing 8 completion tokens with no thinking and every level above it about 114 with thinking. Nothing this model thinks reaches the consumer — the worker browser tab cuts the thinking out, and the local server the native worker talks to reports it in a field of its own that the worker never reads. |
...
Every `POST /v1/chat/completions` request is recorded as one `curl -v`-style block of plain text, appended to its own file, `~/.webai-at-home/consumer_openai/logs/consumer_openai-<run timestamp>.log_http.txt`, one file per run of this server, in the same log directory as the message log described above but kept apart from it: that file is this server's own wire protocol with the central gateway, and this one is what a caller of this server actually experienced. This is the work described by [issue #75](https://github.com/webai-at-home/webai-at-home/issues/75).
...
It follows that this file holds the keys presented to this server and every prompt and answer that went through it, in full. **It is as sensitive as the credentials and the histories it records**, and should be treated that way anywhere it is copied. It is written under `~/.webai-at-home/consumer_openai/logs`, which `--log-dir` moves, and never into the directory this server was started from.
