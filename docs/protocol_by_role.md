...
- `v` is the protocol version the frame was written for. The current version is 10, declared by `protocolVersion` in [`packages/protocol/src/message/envelope_types.ts`](../packages/protocol/src/message/envelope_types.ts). No earlier version is accepted.
...
- `responseFormat` says what shape the answer is to be written in, and carries the schema that shape has to match. It holds `json_object` or `json_schema`, named as the OpenAI Chat Completions interface names them, and a `json_schema` carries the JSON Schema itself rather than the bare name of a shape a worker would then have to invent. Which task type honours which shape is written down once, in [`packages/protocol/src/task/structured_output_support.ts`](../packages/protocol/src/task/structured_output_support.ts), which is a separate table from the one the generation controls are refused against.

Three counts of those settings are used below, and they are nested rather than in disagreement. There are **eight settings** in all. Six of them are **generation controls**: `isStreaming` is left out, because it says how the answer is delivered rather than how it is generated, and `responseFormat` is left out, because it says what shape the answer is written in rather than how it is generated, and is refused against a table of its own. Five of those six are **sampling controls**: `reasoningEffort` is set aside as well, because it budgets a model's thinking rather than steering the choice of the next token.

Each of the six generation controls is carried exactly as the consumer asked for it and is never translated for the engine that will run the task. Which task type honours which control is written down once, in [`packages/protocol/src/task/generation_control_support.ts`](../packages/protocol/src/task/generation_control_support.ts), and what is written there is what a de-risk gate observed live rather than what an engine documents. `task_type_llm_qwen3_0_6b_sharded` honours all five of the sampling controls, because its sampler is written by hand over the logits; `task_type_llm_llama3_2_1b_full` honours `temperature`, `maximumOutputTokenCount`, and `stopSequences`, its `@huggingface/transformers` engine acting on neither `topP` nor a seed; `task_type_llm_qwen3_5_0_8b_full` honours those three and `reasoningEffort` besides, being one of the two task types whose model thinks before it answers on both of its workers; `task_type_llm_gemma_nano_chrome_full` honours none, being the one engine no live run could reach; and `task_type_llm_gemma_4_e2b_full` honours those same four, which was arrived at by measuring that model on both of its workers rather than by copying the row of another task type on the same library. See [issue #180](https://github.com/webai-at-home/webai-at-home/issues/180), [issue #196](https://github.com/webai-at-home/webai-at-home/issues/196), [issue #192](https://github.com/webai-at-home/webai-at-home/issues/192), [issue #222](https://github.com/webai-at-home/webai-at-home/issues/222), and [issue #223](https://github.com/webai-at-home/webai-at-home/issues/223).
