...
Every `parameters.*` test is skipped through `consumer_openai`, while LM Studio honours all five. That is a decision, not an oversight. A task type's declaration in `packages/protocol/src/task/generation_control_support.ts` is the contract of the task type, not the capability of whichever worker happens to take the job — and a task type such as `task_type_llm_llama3_2_1b_full` may be run either by a browser tab that cannot sample or by a native worker that can. A consumer does not choose between them, so the contract stays empty until both workers can honour the same control. The conformance test marks this ⊘ and leaves it out of the percentage, which is the correct reading: nothing was measured, and nothing was broken.
