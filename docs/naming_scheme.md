...
- The sample commands in [`packages/consumer_cli/package.json`](../packages/consumer_cli/package.json) are `sample:dev_formula`, `sample:llm_qwen3_0_6b_sharded`, `sample:llm_gemma_nano_chrome_full`, `sample:llm_qwen3_5_0_8b_full`, and `sample:llm_llama3_2_1b_full`.
- The consumer names those sample commands register under are the same words with underscores written as hyphens, because they are display names rather than identifiers: `dev-formula-consumer`, `llm-qwen3-0-6b-sharded-consumer`, `llm-gemma-nano-chrome-full-consumer`, `llm-qwen3-5-0-8b-full-consumer`, and `llm-llama3-2-1b-full-consumer`.
- A task whose worker is not a browser tab has no debug page of its own, because those pages exist to open worker browser tabs inside inline frames. `task_llm_llama3_2_1b_full` is reached by two different kinds of worker (see the `full` rule above): a worker browser tab, which the debug page below opens, and the Node.js worker in [`packages/worker_openai`](../packages/worker_openai), started from a command line instead, for which no debug page exists — the same task type, one arrangement with a debug page and one without.
- The gateway debug pages are served at `/debug_iframe_dev_formula`, `/debug_iframe_llm_qwen3_0_6b_sharded`, `/debug_iframe_llm_gemma_nano_chrome_full`, `/debug_iframe_llm_qwen3_5_0_8b_full`, and `/debug_iframe_llm_llama3_2_1b_full`, and each inline frame in them is named after the single stage it is restricted to, with underscores written as hyphens: `dev-formula-multiply`, `dev-formula-add`, `llm-qwen3-0-6b-shard1of3`, `llm-qwen3-0-6b-shard2of3`, `llm-qwen3-0-6b-shard3of3`, `llm-gemma-nano-chrome-full`, `llm-qwen3-5-0-8b-full`, and `llm-llama3-2-1b-full`.
- The worker source file that implements the computations of one domain is named after those computations, as `stage_helper_{task_suffix}.ts`: [`packages/worker_webpage/web/src/stages/stage_helper_dev_formula.ts`](../packages/worker_webpage/web/src/stages/stage_helper_dev_formula.ts), [`packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_0_6b_sharded.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_0_6b_sharded.ts), [`packages/worker_webpage/web/src/stages/stage_helper_llm_gemma_nano_chrome_full.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_gemma_nano_chrome_full.ts), and [`packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_5_0_8b_full.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_5_0_8b_full.ts). A worker that is not a browser names its own stage files the same way, in its own package. `task_llm_llama3_2_1b_full` is the one case where two different worker packages each name a stage helper file after the same computation, because either can run it: [`packages/worker_webpage/web/src/stages/stage_helper_llm_llama3_2_1b_full.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_llama3_2_1b_full.ts), holding `StageHelperLlmLlama3_2_1bFull` for the worker browser tab that downloads and runs the model, and [`packages/worker_openai/src/stages/stage_helper_llm_llama3_2_1b_full.ts`](../packages/worker_openai/src/stages/stage_helper_llm_llama3_2_1b_full.ts), holding a class of the same name for the native worker that forwards the prompt to a local server instead.
- `packages/consumer_openai/tests/` names its real end-to-end test files `real_{task_suffix}.test.ts`, one file per task type, except for `task_llm_llama3_2_1b_full`: since two different worker types can run it, `real_llm_llama3_2_1b_full.test.ts` drives the worker browser tab and `real_llm_llama3_2_1b_full_native_worker.test.ts` drives the native `@webai/worker-openai` worker.
