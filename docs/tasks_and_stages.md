...
This task type is reachable by two different kinds of worker, the same arrangement `task_type_llm_llama3_2_1b_full` describes at greater length: the worker browser tab above, and a native worker from [`packages/worker_openai`](../packages/worker_openai) that forwards the prompt to a local server already running on its own device that speaks the OpenAI-compatible Chat Completions API, such as [LM Studio](https://lmstudio.ai). Nothing in the paragraphs above about downloading, loading, quantization, or storage applies to that second kind of worker: the local server holds the model, loads it, and drives the hardware, and which server is reached and which model it is asked for are options of the worker process, `--openai-base-url` and `--openai-model`. The gateway does not know or care which of the two a given assignment reaches: both advertise the same `stage_llm_qwen3_5_0_8b_full`.
...
**What it does:** it generates text with the complete Llama 3.2 1B Instruct language model, and is reachable by two different kinds of worker, as `task_type_llm_qwen3_5_0_8b_full` also is, following the `naming_scheme.md` rule that `full` names an arrangement rather than which program holds the model:

- A worker browser tab downloads the model directly from Hugging Face and holds it entirely itself, the same arrangement `task_type_llm_qwen3_5_0_8b_full` uses. The asset downloaded and run is [`onnx-community/Llama-3.2-1B-Instruct-ONNX`](https://huggingface.co/onnx-community/Llama-3.2-1B-Instruct-ONNX), the ONNX export of `meta-llama/Llama-3.2-1B-Instruct` — the base model is a gated repository that a browser tab carrying no Hugging Face access token cannot download, but this export is not gated, which is what makes the task possible at all. It is pinned to revision `14007543b6dc92de88daf96a9aa85d2f95ace6ef`, at the `q4f16` quantization, licensed under the Llama 3.2 Community License (`llama3.2`) rather than the Apache 2.0 licence `onnx-community/Qwen3.5-0.8B-ONNX` carries — a different licence from every other model this project runs, which carries its own attribution and naming requirements. The architecture is `LlamaForCausalLM`, an ordinary transformer with 16 hidden layers, 32 attention heads, 8 key-value heads, and a plain `past_key_values` cache on every layer — not a hybrid architecture the way Qwen3.5-0.8B is. This task's worker computation is still built on [`@huggingface/transformers`](https://www.npmjs.com/package/@huggingface/transformers) rather than on hand-built ONNX Runtime Web feeds, but for a different reason than Qwen3.5-0.8B's: the sharded decode loop this project wrote by hand exists to split a model across several devices, and this model is not split.
- A native worker from [`packages/worker_openai`](../packages/worker_openai) forwards the prompt to a local server already running on its own device that speaks the OpenAI-compatible Chat Completions API, such as [LM Studio](https://lmstudio.ai), asked for the model `llama-3.2-1b-instruct` by default. This is the same arrangement `task_type_llm_llama3_2_3b_full` used before it was retired, now serving the same task type a worker browser tab serves rather than a task type of its own. Which server the worker talks to, and which model it asks that server for, are options of the worker process, `--openai-base-url` and `--openai-model`.

The gateway does not know or care which of the two kinds of worker a given assignment reaches: both advertise the same `stage_llm_llama3_2_1b_full`, and the gateway assigns it to whichever worker offers it.

Whichever kind of worker runs it, the answer is produced the same way `task_type_llm_qwen3_5_0_8b_full` produces one, in pieces or as a whole according to the `isStreaming` generation setting the consumer submitted:
...
A worker browser tab stops generation on one of the model's own three `eos_token_id` values (`128001`, `128008`, and `128009`), or on a safety bound of generated tokens, defined in [`packages/worker_webpage/web/src/stages/stage_helper_llm_llama3_2_1b_full.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_llama3_2_1b_full.ts). A native worker stops when the local server finishes the answer, or on a safety bound of read pieces, defined in [`packages/worker_openai/src/stages/local_server_generation.ts`](../packages/worker_openai/src/stages/local_server_generation.ts) — there is no bound on generated tokens on that path, because the number of tokens is the local server's own decision rather than a value this project passes.

**Purpose:** it is the second complete-model task after Qwen3.5-0.8B that this project downloads and runs itself in a worker browser tab, and it proves a task type is not tied to one kind of worker: the same pipeline, the same stage, and the same consumer-facing name are served equally by a browser tab running the model and by a native process forwarding to a local server already running it.
...
The stage sets `prefersSameWorkerOnRetry`. An answer read in pieces lives in the memory of the one worker producing it — the downloaded model and generation state for a worker browser tab, an open request to the local server for a native worker — so every run of it has to reach that same worker, the same reasoning `stage_llm_gemma_nano_chrome_full` follows and the same release, idle-timeout, and replaced-run handling it uses.
...
```bash
npm run sample:llm_llama3_2_1b_full --workspace @webai/consumer-cli
```
...
This task type honours `temperature`, `maximumOutputTokenCount`, and `stopSequences`, named for it in [`packages/protocol/src/task/generation_control_support.ts`](../packages/protocol/src/task/generation_control_support.ts) and acted on since [issue #222](https://github.com/webai-at-home/webai-at-home/issues/222). A request that asks for none of them generates greedily, which is what every request did before that issue. `topP` and `randomSeed` are refused, because that issue's live run found the engine reads neither, and no control may be acted on until that table names it. It honours `reasoningEffort` as well, since [issue #223](https://github.com/webai-at-home/webai-at-home/issues/223): the chat template reads `enable_thinking`, rendering one history as 22 tokens with thinking off and 29 with it on, and the local server the native worker talks to reads `reasoning_effort` on the request. Both workers are as coarse as each other — `none` is off, every level above it is on — and a request that asks for nothing still does not think, which is what this stage did before the control was settable. Nothing the model thinks reaches the consumer: the worker browser tab cuts the thinking out on the token identifiers — out of the answer, and out of the text a tool call is looked for in, so a call the model only sketched while working out what to do is never reported as one it decided on — and the local server reports the thinking in a field of its own that the native worker never reads. An answer that thought is never reported one piece at a time, because where the thinking ends is only known once it has ended. Thinking off by default stays a measured choice rather than an arbitrary one — measured on a local model server, this model with thinking on answered 515 characters to a request budgeted at 8 tokens.
